...
def train(classifier, criterion, device, batch_size=4, num_workers=2, epoch_num=2, learning_rate=4e-2, gamma=0.9997):
    train_set = torchaudio.datasets.GTZAN(r"C:\Users\elata\code\MusicGenreClassifier\datasets\genres", subset="training")
    train_data = torch.utils.data.DataLoader(train_set,
                                batch_size=batch_size,
                                shuffle=True,
                                num_workers=num_workers)
    val_set = torchaudio.datasets.GTZAN(r"C:\Users\elata\code\MusicGenreClassifier\datasets\genres", subset="validation")
    ...
    for epoch in range(epoch_num):
        train_loss = train_epoch(classifier, train_data, criterion, optimizer, device, length)
